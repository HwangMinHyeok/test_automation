...
QA 자동화 직무 지원을 위해 테스트 자동화 연습용 웹사이트인 [Automation Exercise](https://automationexercise.com/)를 대상으로 Playwright와 pytest-bdd 기반의 E2E 테스트 자동화 구조를 구현한 프로젝트입니다.
...
Python의 requests 라이브러리를 사용해 상품 검색 API의 응답 데이터를 조회하고, 동일한 검색 조건으로 UI에 표시되는 상품 데이터와 비교했습니다. 이를 통해 API 응답과 실제 화면 표시 데이터가 일관되게 제공되는지 검증했습니다.
...
프로젝트를 진행하면서 오류 분석, 코드 작성 보조, 문서 작성 등에 AI 도구를 활용했습니다.

- ChatGPT: 프로젝트 구상, 각종 검색
- Codex: 프로젝트 구조 검토 및 개선 방향 제안, README.md 등 문서 작성 보조
- GitHub Copilot in VSCode: 프로젝트 맥락에 맞는 코드 추천 및 자동완성 지원
